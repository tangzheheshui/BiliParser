...
### 迭代需求（v11）：官网地址纠正——修掉哑掉的更新提示（2026-09-11，0.2.6）

已发布的 **0.2.5 包内烧的是旧官网地址** `http://tangzheheshui.cn/biliparser`，
该地址实测 **404**，于是：

- 品牌名点击的「官网」链接打开即 404
- `api_update_check` 拉 `<官网>/download/version.json` 得 404 → `r.json()` 抛异常
  被 `except` 吞掉 → 恒 `update_available=false` → **装 0.2.5 的用户永远收不到
  更新提示**，发再多新版本也通知不到

根因：该地址在 819b5d7（2026-09-07）才改为 `https://biliparser.tangzheheshui.cn`，
而 v0.2.5 的 tag 打在这之前，改动未进任何已发布包。**0.2.6 = 0.2.5 + 这一行**。
注意老用户收不到自动提示，须由卖码渠道人工知会重下。
